    ...
    private void salvarTiquetesJSON(Aerolinea aerolinea, JSONObject jobject) throws JSONException {
        ...
        for (Cliente cliente : clientes) {
            for (Tiquete tiquete : cliente.getTiquetesSinUsar()) {
                ...
                jsonTiquete.put("usado", false);
                jsonTiquete.put("cliente", cliente.getIdentificador());
                jsonTiquete.put("fecha", tiquete.getVuelo().getFecha());
                jsonTiquete.put("codigoRuta", tiquete.getVuelo().getRuta().getCodigoRuta());
                tiquetesArray.put(jsonTiquete);
            }
            for (Tiquete tiquete : cliente.getTiquetesUsados()) {
                JSONObject jsonTiquete = new JSONObject();
                jsonTiquete.put("codigoTiquete", tiquete.getCodigo());
                jsonTiquete.put("tarifa", tiquete.getTarifa());
                jsonTiquete.put("usado", true);
                jsonTiquete.put("cliente", cliente.getIdentificador());
                jsonTiquete.put("fecha", tiquete.getVuelo().getFecha());
                jsonTiquete.put("codigoRuta", tiquete.getVuelo().getRuta().getCodigoRuta());
                tiquetesArray.put(jsonTiquete);
